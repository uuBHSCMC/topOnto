...
ks.test.2 <- function (x, y, ..., alternative = c("two.sided", "less", "greater"), exact = NULL, maxCombSize=10000) 
{
  ...
  #now call the original ks.test
  KSorg<-ks.test(x, y,alternative = alternative, exact = exact) 
  RVAL <- list(statistic = KSorg$statistic, p.value = KSorg$p.value, alternative = KSorg$alternative, 
               method = KSorg$method, data.name = KSorg$data.name, ES = ES, edge = edge)
  class(RVAL) <- "htest"
  return(RVAL)
}

ks.test.3 <- function (x, y, ..., alternative = c("two.sided", "less", "greater"), exact = NULL, maxCombSize=10000) 
{
...
#      PVAL <- ks.test(x,y,alternative = "two.sided",exact=TRUE)$p.value
...
#      PVAL <- ks.test(x,y,alternative = alternative,exact=exact)$p.value
      PVAL <- 1 - if (alternative == "two.sided")
        ...
        PVAL <- 1 - pkolmogorov1x(STATISTIC, n)
      }
    }
    nm_alternative <- switch(alternative, two.sided = "two-sided", 
